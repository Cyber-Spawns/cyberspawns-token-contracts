...
contract CyberSpawnPresale is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant TH = 20; // 100% / 5%
    uint256 public constant MAX_OWNABLE = 6;

    uint8 public constant CLASS_AVES = 0;
    uint8 public constant CLASS_MAMMALS = 1;
    uint8 public constant CLASS_REPTILES = 2;
    uint8 public constant CLASS_MOLLUSC = 3;
    uint8 public constant CLASS_AQUA = 4;
    ...
    function buySpawns(
        ...
        require(
            whitelist[recipient] || onPresale,
            "whitelist or can buy in presale peroid"
        );

        // check if exceed one buy limit
        uint256 _amount = cyberspawn.balanceOf(recipient);
        require(
            _amount.add(aves).add(mammals).add(reptiles).add(mollusc).add(
                aqua
            ) < MAX_OWNABLE,
            "exceed limit of ownable for one address"
        );
        //whitelisted address can buy
        uint256 buyAmount = aves.add(mammals).add(reptiles).add(mollusc).add(
            aqua
        );
        if (whitelist[recipient]) {
            whiteAmount = whiteAmount > buyAmount ? whiteAmount - buyAmount : 0;
        } else {
            uint256 totalSold = sold[CLASS_AVES]
                .add(sold[CLASS_MAMMALS])
                .add(sold[CLASS_REPTILES])
                .add(sold[CLASS_MOLLUSC])
                .add(sold[CLASS_AQUA]);
            require(
                totalSold.add(buyAmount).add(whiteAmount) <= maxAmount.mul(5),
                "sold out"
            );
        }
        ...
        if (mammals > 0) {
            require(
                sold[CLASS_MAMMALS].add(mammals) <= maxAmount,
                "exceed balance"
            );
            payAmount = payAmount.add(
                _buySpawns(recipient, CLASS_MAMMALS, mammals)
            );
        }

        if (reptiles > 0) {
            require(
                sold[CLASS_REPTILES].add(reptiles) <= maxAmount,
                "exceed balance"
            );
            payAmount = payAmount.add(
                _buySpawns(recipient, CLASS_REPTILES, reptiles)
            );
        }

        if (mollusc > 0) {
            require(
                sold[CLASS_MOLLUSC].add(mollusc) <= maxAmount,
                "exceed balance"
            );
            payAmount = payAmount.add(
                _buySpawns(recipient, CLASS_MOLLUSC, mollusc)
            );
        }
    ...
    function spawnPrice(uint8 _spawnType) external view returns (uint256) {
        ...
        uint256 currentPrice = sold[_spawnType].div(bundle).mul(rate).add(
            initialPrice
        );
        return currentPrice;
    }
        ...
        )
    {
        ...
        return (
            totalRemainings,
            whiteAmount,
            maxAmount.sub(sold[CLASS_AVES]),
            maxAmount.sub(sold[CLASS_MAMMALS]),
            maxAmount.sub(sold[CLASS_REPTILES]),
            maxAmount.sub(sold[CLASS_MOLLUSC]),
            maxAmount.sub(sold[CLASS_AQUA])
        );
    }
    ...
    function _buySpawns(
        ...
        spawns[recipient][_spawnType] = spawns[recipient][_spawnType].add(
            amount
        );
    ...
    function _spawnPrice(uint8 _spawnType, uint256 amount)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 currentPrice = sold[_spawnType].div(bundle).mul(rate).add(
            initialPrice
        ); // currentPrice = initialPrice + (soldAmount / 230) * rate
        ...
        for (uint256 curTh = firstTh + 1; curTh <= lastTh; curTh++) {
            if (curTh != lastTh) {
                payAmount = payAmount.add(
                    rate.mul(curTh.sub(firstTh)).mul(bundle)
                );
            } else {
                uint256 restAmount = sold[_spawnType].add(amount).mod(bundle);
                payAmount = payAmount.add(
                    rate.mul(curTh.sub(firstTh)).mul(restAmount)
                );
            }
        }
